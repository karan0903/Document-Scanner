...
if __name__ == "__main__":
    ...
    # Scan all valid images in directory specified by command line argument --images <IMAGE_DIR>
        ...
        im_pdf_files = [f for f in os.listdir(im_dir) if f.endswith('pdf')]
        for im in im_files:
            scanner.scan(im_dir + '/' + im)
        for file in im_pdf_files:
            shutil.copy(im_dir+'/'+file,'output2')
        for file in os.scandir('output'):
            if get_ext(file) in valid_formats:

               

                # to crop also, it will need a signature at the top
                img_rgb = cv2.imread('output/'+file.name)
                img_gray = cv2.cvtColor(img_rgb, cv2.COLOR_BGR2GRAY)
                template = cv2.imread('sign.jpg',0)
                ...
                if len(loc[0]):
                    y1 = loc[0][0]
                    x1 = loc[1][0]
                    y2 = loc[0][len(loc[0])-1]
                    x2 = loc[1][len(loc[1])-1]
                    ...
                    img = cv2.imread('output/'+file.name)
                    crop_img = img[y1:y2+150, x1:x2+350]
                    cv2.imwrite('output2/'+file.name, crop_img,[int(cv2.IMWRITE_JPEG_QUALITY), 10])
                else:
                     # without any signature at the top use this (it won't crop)
                    image = Image.open('output/'+file.name)
                    image.save('output2/'+file.name,quality=10,optimize=True)
